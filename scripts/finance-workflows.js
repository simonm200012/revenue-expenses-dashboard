...
function receiptQueueCard(r){const p=r.extracted||{};return `<div class="wf-row"><div><strong>${wfEscape(p.merchant||r.original_name)}</strong><small>${wfEscape(r.original_name)}${r.mime_type==='application/pdf'?' · Page '+r.page_number:''} · ${wfEscape(p.invoice_date||p.date||'Date needs review')} · Invoice ${wfEscape(p.invoice_number||'number needs review')} · ${(p.line_items||[]).length} items · ${r.status==='linked'?'Attached to transaction':r.status==='dismissed'?'Archived':'Needs review'}</small></div><strong>${p.amount==null?'Check total':'€'+Number(p.amount).toFixed(2)}</strong><button class="page-btn" data-id="${wfEscape(r.id)}" onclick="openStoredReceipt(this.dataset.id)">${r.status==='linked'?'View':'Review'}</button><button class="page-btn" data-id="${wfEscape(r.id)}" onclick="openInvoiceDetails(this.dataset.id)">Invoice & items</button>${r.status==='pending'?`<button class="page-btn" data-id="${wfEscape(r.id)}" onclick="archiveReceipt(this.dataset.id,true)">Archive</button>`:''}${r.status==='dismissed'?`<button class="page-btn" data-id="${wfEscape(r.id)}" onclick="archiveReceipt(this.dataset.id,false)">Reopen</button>`:''}</div>`;}
...
function disposePrivateReceipts(){if(typeof clearInvoiceState==='function')clearInvoiceState();workflowEpoch++;activeReceiptId=null;FINANCE_RECEIPTS=[];FINANCE_BACKUPS=[];FINANCE_ROWS=[];for(const url of privatePreviewURLs)URL.revokeObjectURL(url);privatePreviewURLs.clear();document.getElementById('privateReceiptDialog')?.close();}
...
async function openStoredReceipt(id){
 ...
 document.getElementById('qaInvoiceDetails').hidden=false;
